...
public class FolderDialog extends JDialog {
	...
	public static void main(String[] args) {
		try {
			FolderDialog dialog = new FolderDialog(false);
	 ...
	 * Create the dialog.
	 */
	public FolderDialog(boolean lua) {
		setModal(true);
		...
		getContentPane().add(contentPanel, BorderLayout.CENTER);
		{
			...
			if (lua==false){
				for (int i=0; i<IndexOrthanc.pluginsFolder.size();i++){
					model.addRow(new Object [] {IndexOrthanc.pluginsFolder.get(i).toString()});
				}
			}
			//Si on est dans Lua on traite la variable Lua
			if (lua==true){
				for (int i=0; i<IndexOrthanc.luaFolder.size();i++){
					model.addRow(new Object [] {IndexOrthanc.luaFolder.get(i).toString()});
				}
			}
			...
			}
			{
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						if (lua) IndexOrthanc.luaFolder.clear();
						if (!lua) IndexOrthanc.pluginsFolder.clear();
						DefaultTableModel model = (DefaultTableModel) table.getModel();
						for (int i=0; i<model.getRowCount();i++){
							if (!lua) IndexOrthanc.addplugins(model.getValueAt(i, 0).toString());
