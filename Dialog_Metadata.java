...
public class Dialog_Metadata extends JDialog {
	...
	public static void main(String[] args) {
		try {
			Dialog_Metadata dialog = new Dialog_Metadata();
	...
	public Dialog_Metadata() {
		...
		contentPanel.setLayout(new BorderLayout(0, 0));
		{
			...
			contentPanel.add(scrollPane);
			{
				...
				if (IndexOrthanc.userMetadata.size()>0){
					String[] metadata=new String[IndexOrthanc.userMetadata.size()];
					IndexOrthanc.userMetadata.keySet().toArray(metadata);
					for (int i=0; i<metadata.length; i++){
						JSONArray meta=(JSONArray) IndexOrthanc.userMetadata.get(metadata[i]);
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						for (int i=0; i<table.getRowCount(); i++){
							IndexOrthanc.addUserMetadata(table.getValueAt(i, 0).toString(), Integer.valueOf(table.getValueAt(i, 1).toString()));
						}
