...
public class TableDataExportSeries extends AbstractTableModel{
	...
	public TableDataExportSeries(String[] parametresTableau, JFrame frame, JLabel stateExport){
		super();
	...
	public boolean checkSopClassUid(String instanceUid) throws IOException{
		if(instanceUid != null){
			url=fullAddress+"/instances/" + instanceUid + "/metadata/SopClassUid";
	...
	public void removeAllSecondaryCaptures() throws IOException{
		...
		for(Serie s : series){
			if(s.isSecondaryCapture()){
				url=fullAddress+"/series/" + s.getId();
				URL url = new URL(this.url);
				HttpURLConnection conn = (HttpURLConnection) url.openConnection();
				conn.setDoOutput(true);
